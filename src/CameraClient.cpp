...
{
    return cv::Mat(data.size() - offset, 1, cv::DataType<uchar>::type, const_cast<char*>((data.data() + offset)));
}
...
cv::Mat read32FC1Mat(const std::string& data, double scale)
{
    if (data.empty())
        return {};
    cv::Mat bias16U = cv::imdecode(asMat(data), cv::ImreadModes::IMREAD_ANYDEPTH);
    cv::Mat bias32F = cv::Mat::zeros(bias16U.size(), CV_32FC1);
    bias16U.convertTo(bias32F, CV_32FC1);
    cv::Mat mat32F = bias32F + cv::Mat(bias32F.size(), bias32F.type(), cv::Scalar::all(-Encode32FBias));
...
cv::Mat read32FC3Mat(const std::string& data, double scale)
{
    ...
    cv::Mat depth32F = bias32F + cv::Mat(bias32F.size(), bias32F.type(), cv::Scalar::all(-Encode32FBias));
